...
fn image_shape(
    ...
    if arg_shape.0.unwrap_or(0) > num_pixels {
        Err("Width is too large.")?
    }
    if arg_shape.1.unwrap_or(0) > num_pixels {
        Err("Height is too large.")?
    }
    ...
    match arg_shape {
        ...
        (None, Some(height)) => {
            let width = int_ceil(num_pixels, height);
            Ok((width, height))
        }
        _ => unreachable!(),
    }
...
/// `colortype` - The type of pixel to use
fn bytes_to_add(buffer_size: usize, dims: (u32, u32), colortype: ColorType) -> usize {
    ...
    return bytes_required as usize - buffer_size;
}
...
fn render_file(args: Args) -> Result<(), &'static str> {
    // Read in binary file
    let mut file = File::open(&args.arg_input).map_err(|_| "Couldn't open input file.")?;
    ...
    // Fill in last row of pixels with black if necessary
    let size_diff = bytes_to_add(file_size, dims, colortype);
    let mut zeros = vec![0; size_diff];
    buffer.append(&mut zeros);

    // Write image
    let output_path = args.arg_output.unwrap_or("out.png".to_string());
    image::save_buffer(
        &output_path,
        &buffer,
        dims.0,
